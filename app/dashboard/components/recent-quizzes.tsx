...
export function RecentQuizzes() {
  ...
  const [userStats, setUserStats] = useState<{ solvedQuiz: QuizItem[] } | null>(null)
  ...
  return (
    <div className="space-y-4">
      {userStats?.solvedQuiz?.map((quiz) => {
        ...
        const score = total > 0 ? Math.round((quiz.correctQuizAnswer / total) * 100) : 0

        return (
          <div key={quiz._id} className="flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-sm font-medium leading-none">{quiz.quizCriteria.topic}</p>
              <div className="flex items-center gap-2">
                <p className="text-xs text-muted-foreground">{quiz.quizCriteria.quizType.toUpperCase()}</p>
                <Badge
                  variant={
            ...
            <div
              className={`font-medium text-sm ${
                score >= 90 ? "text-green-600" : score >= 70 ? "text-blue-600" : "text-orange-600"
              }`}
            >
              {score}%
            </div>
